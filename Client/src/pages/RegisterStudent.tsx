...
const RegisterStudent: React.FC = () => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setStudent((prev) => ({
      ...prev,
      [name]: name === 'sId' ? value.replace(/\D/g, '').slice(0, 9) : value
    }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await studentService.register(student as Student);
      setStatus({ msg: 'התלמידה נרשמה בהצלחה!', type: 'success' });
      setTimeout(() => navigate('/monitor'), 2000);
    } catch (err: any) {
      let serverError = 'ארעה שגיאה בתהליך הרישום';
      ...
      if (err.response && err.response.data) {
        if (typeof err.response.data === 'string') {
          ...
          serverError = JSON.stringify(err.response.data);
        }
      }
        ...
        <Row className="justify-content-center">
          <Col md={8} lg={6}>
            <Card className="p-4 shadow-sm border-0" style={{ borderRadius: '20px' }}>
              <h3 className="text-center mb-4 fw-bold" style={{ color: '#28a745' }}>רישום תלמידה לטיול</h3>
              ...
              <Form onSubmit={handleSubmit}>
                <Form.Group className="mb-3">
                  ...
                  <Form.Control name="sId" value={student.sId} onChange={handleChange} placeholder="הזיני 9 ספרות" required />
                </Form.Group>
                ...
                <Form.Group className="mb-4">
                  ...
                  <Form.Control name="className" value={student.className} onChange={handleChange} placeholder="למשל: ח' 2" required />
                </Form.Group>

                <Button variant="success" size="lg" className="w-100 py-3 fw-bold shadow-sm" style={{ borderRadius: '50px' }} type="submit" disabled={loading}>
                  {loading ? <Spinner size="sm" animation="border" /> : 'בצע רישום'}
                </Button>
                
                <Button variant="link" className="w-100 mt-2 text-decoration-none text-muted small" onClick={() => navigate('/')}>
                  ביטול וחזרה
                </Button>
              </Form>
            </Card>
